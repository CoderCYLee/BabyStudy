...
- (void)createUI
{
    ...
    self.myScrollerview = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, screenW, screenH)];
    self.myScrollerview.delegate=self;
    self.myScrollerview.bounces = NO;
    [self.view addSubview:self.myScrollerview];
    
    self.myScrollerview.contentSize = CGSizeMake(screenW, 200*6);
    ...
    for(int j=0;j<6;j++) {
        for(int k=0;k<3;k++) {
            ...
            [_myScrollerview addSubview:imageview1];
            ...
            button.tag = j*3+k+1;
            [_myScrollerview addSubview:button];
            
            NSString *str1 = [_dataArray objectAtIndex:j*3+k];
            ...
            [_myScrollerview addSubview:label];
        }
    }
}
